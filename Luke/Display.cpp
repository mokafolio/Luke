...
Display::Display() : m_pimpl(makeUnique<detail::DisplayImpl>(defaultAllocator()))
{
}
...
Display::Display(const Display & _other) : m_pimpl(_other.m_pimpl->clone())
{
}
...
Error Display::setDisplayMode(const DisplayMode & _mode)
{
    ...
    // else
    //     _error = ErrorReport(error::InvalidDisplayMode, "The display mode is unsupported by this
...
DisplayMode Display::findBestDisplayMode(Float32 _width,
                                         Float32 _height,
                                         UInt32 _redBits,
                                         UInt32 _greenBits,
                                         UInt32 _blueBits,
                                         UInt32 _refreshRate) const
{
    ...
    for (auto & mode : modes)
    {
        ...
        {
            if (_redBits == mode.redBits() && _greenBits == mode.greenBits() &&
                _blueBits == mode.blueBits())
            {
