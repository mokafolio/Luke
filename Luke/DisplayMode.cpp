...
DisplayMode::DisplayMode(Float32 _width,
                         Float32 _height,
                         stick::UInt32 _redBits,
                         stick::UInt32 _greenBits,
                         stick::UInt32 _blueBits,
...
bool DisplayMode::isValid() const
{
    return m_width > 0 && m_height > 0 &&
           (m_refreshRate == RefreshRate::DontCare || m_refreshRate > 0) && m_redBits > 0 &&
           m_greenBits > 0 && m_blueBits > 0;
}

bool DisplayMode::operator==(const DisplayMode & _other) const
{
    return m_refreshRate == _other.m_refreshRate && m_redBits == _other.m_redBits &&
           m_greenBits == _other.m_greenBits && m_blueBits == _other.m_blueBits &&
           m_width == _other.m_width && m_height == _other.m_height;
}
...
bool DisplayMode::operator<(const DisplayMode & _other) const
{
    ...
    {
        if (m_redBits == _other.m_redBits && m_greenBits == _other.m_greenBits &&
            m_blueBits == _other.m_greenBits)
        {
