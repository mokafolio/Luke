...
DisplayImpl::DisplayImpl() : m_glfwMonitor(NULL)
{
}
...
Error DisplayImpl::setDisplayMode(const DisplayMode & _mode)
{
    ...
    return Error(
        ec::Unsupported,
        "GLFW only allows you to change the display mode when entering fullscreen with a window",
        STICK_FILE,
        STICK_LINE);
}
...
DisplayModeArray DisplayImpl::displayModes() const
{
    ...
    {
        ret.append(DisplayMode(modes[i].width,
                               modes[i].height,
                               modes[i].redBits,
                               modes[i].greenBits,
                               modes[i].blueBits,
